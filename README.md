...
├── outputs/
│   ├── 00_正式主结果_Predicted-0.75_论文当前使用/  # 当前论文正式主结果（推荐优先查看）
│   │   ├── 01_系统级主结果/
│   │   ├── 02_Scale对比_0_0.75_0.90/
│   │   ├── 03_条件有效性_完整版/
│   │   ├── 04_贡献分解_ZeroCondition对照/
│   │   ├── 05_统计显著性_改善比例_分布/
│   │   ├── 06_典型样本频谱图/
│   │   ├── 07_复杂度与CPU运行成本/
│   │   └── 08_汇总文档与回填清单/
│   ├── 01_阶段性打包结果_历史参考勿直接写论文/   # 历史打包结果，保留参考
│   ├── 02_旧版结果备份_更新前保留/              # 更新前旧结果备份
│   ├── 03_归档结果_archive_更早期实验/          # 更早期实验归档
│   ├── 04_评测缓存与清单_复现实验用/            # 缓存与 manifest
│   └── 05_待清理_中间产物与旧小清单/            # 待后续确认删除的中间物
└── README.md
```
...
如果你当前的目标是**查阅论文正式主结果**，建议按以下顺序查看：

1. `outputs/00_正式主结果_Predicted-0.75_论文当前使用/01_系统级主结果/`
2. `outputs/00_正式主结果_Predicted-0.75_论文当前使用/02_Scale对比_0_0.75_0.90/`
3. `outputs/00_正式主结果_Predicted-0.75_论文当前使用/03_条件有效性_完整版/`
4. `outputs/00_正式主结果_Predicted-0.75_论文当前使用/04_贡献分解_ZeroCondition对照/`
5. `outputs/00_正式主结果_Predicted-0.75_论文当前使用/05_统计显著性_改善比例_分布/`
6. `outputs/00_正式主结果_Predicted-0.75_论文当前使用/06_典型样本频谱图/`
7. `outputs/00_正式主结果_Predicted-0.75_论文当前使用/08_汇总文档与回填清单/`

历史打包结果、旧版结果与中间产物已分别迁移到带中文说明的新目录中，避免与正式主结果混淆。
