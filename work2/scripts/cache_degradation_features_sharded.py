"""
Shard-based feature cache for GTCRN degradation features.

This version stores many samples in one shard file to avoid the severe
Windows filesystem overhead caused by hundreds of thousands of tiny .pt files.
"""
import argparse
import json
import sys
...
def _flush_shard(shard_buffer: list[dict], out_dir: Path, shard_id: int) -> tuple[str, int]:
    ...
    shard_file = f"shard_{shard_id:05d}.pt"
    shard_path = out_dir / shard_file

    stats = torch.stack([x["stats"] for x in shard_buffer], dim=0)
    ...
    seed_list = [x["seed"] for x in shard_buffer]
    split_list = [x["split"] for x in shard_buffer]
...
def main():
    parser = argparse.ArgumentParser(description="Cache GTCRN degradation features into shard files")
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/estimator_cache_sharded")
    ...
    print(f"[INFO] Feature extractor loaded in {time.perf_counter()-t0:.1f}s. Stats dim = {stats_dim}")
    ...
    shard_buffer = []
    shard_id = len(list(out_dir.glob("shard_*.pt")))
    extraction_times = []
    total_new = 0
    ...
    for src_idx, entry in enumerate(manifest_lines):
        ...
        for sev_idx, sev in enumerate(SEVERITIES):
            ...
            extraction_times.append(time.perf_counter() - t_ext)
            ...
            if len(shard_buffer) >= args.shard_size:
                ...
                print(f"[INFO] Flushed shard {shard_id-1:05d}, total cached={len(index_entries)}")

        if src_idx % 500 == 0:
            print(f"[INFO] Progress: {src_idx}/{n_sources} sources, {len(existing)} cached samples")
    ...
    if shard_buffer:
        ...
        with open(index_path, "w", encoding="utf-8") as f:
            for e in index_entries:
                f.write(json.dumps(e, ensure_ascii=False) + "\n")
        print(f"[INFO] Flushed final shard {shard_id:05d}, total cached={len(index_entries)}")
    ...
    cache_size_mb = sum(f.stat().st_size for f in out_dir.glob("shard_*.pt")) / (1024 * 1024)
    summary = {
        ...
        "mean_extraction_time_s": round(float(np.mean(extraction_times)), 4) if extraction_times else 0.0,
        "seed": args.seed,
        "shard_size": args.shard_size,
        "n_shards": len(list(out_dir.glob('shard_*.pt'))),
    }
    with open(out_dir / "cache_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)

    print(f"[INFO] Cached {len(index_entries)} samples")
