...
def main():
    ...
    with torch.inference_mode():
        for shard_idx, shard_file in enumerate(shard_files, start=1):
            ...
            clean_batch = shard["clean_spec"][offsets].float()       # (N,F,T,2)
            degraded_batch = shard["degraded_spec"][offsets].float() # (N,F,T,2)
            base_batch = shard["enhanced_base"][offsets].float()     # (N,F,T,2)
            ...
            v1_batch = v1_out["enhanced_final"].permute(0, 3, 2, 1).contiguous().cpu()
            ...
            for local_idx, off in enumerate(offsets):
                ...
                v1_spec = v1_batch[local_idx].numpy()
    ...
    with open(out_dir / "table_enhancement_by_severity.csv", "w", newline="", encoding="utf-8-sig") as f:
        ...
        for sev in SEVERITIES:
            ...
            for m in methods:  # include degraded/base/v1/v2
    ...
    def plot_severity_metric(metric_key, ylabel, filename, higher_better=True):
        ...
        ax.bar(x + width/2, v1_vals, width, label="EDCR-V1")
        ax.set_xticks(x)
