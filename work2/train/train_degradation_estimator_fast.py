"""
Fast shard-based training for the degradation estimator.
"""
import argparse
import json
import sys
...
def compute_metrics(predictions, targets):
    ...
    return {"noise_acc": noise_acc, "bandwidth_acc": bw_acc, "bit_acc": bit_acc, "snr_mae_db": snr_mae}


def run_epoch(model, loader, loss_fn, optimizer, device, train=True, log_interval=100):
    if train:
        model.train()
    else:
        model.eval()
    ...
    with context:
        for step, batch in enumerate(loader, start=1):
            ...
            preds = model(stats)
            loss, comps = loss_fn(preds, targets)
            if train:
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="outputs/degradation_estimator_fast")
    ...
    train_ds = DegradationFeatureShardDataset(str(index_path), str(cache_dir), split="train")
    valid_ds = DegradationFeatureShardDataset(str(index_path), str(cache_dir), split="valid")
    ...
    if args.resume and (out_dir / "last_model.pt").exists():
        ckpt = torch.load(out_dir / "last_model.pt", map_location=device)
        ...
        model = DegradationEstimator(input_dim=train_ds.stats_dim).to(device)

    n_params = model.count_trainable_params()
    print(f"[INFO] Stats dimension: {train_ds.stats_dim}")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        train_metrics = run_epoch(model, train_loader, loss_fn, optimizer, device, train=True, log_interval=args.log_interval)
        valid_metrics = run_epoch(model, valid_loader, loss_fn, optimizer, device, train=False, log_interval=max(args.log_interval, 1000))
        scheduler.step(valid_metrics["loss"])

        epoch_info = {"epoch": epoch, "train": train_metrics, "valid": valid_metrics}
        history.append(epoch_info)

        print(f"Epoch {epoch:3d}/{args.epochs} | train_loss={train_metrics['loss']:.4f} | valid_loss={valid_metrics['loss']:.4f} | noise_acc={valid_metrics['noise_acc']:.3f} | bw_acc={valid_metrics['bandwidth_acc']:.3f} | bit_acc={valid_metrics['bit_acc']:.3f}" + (f" | snr_mae={valid_metrics['snr_mae_db']:.2f}dB" if valid_metrics.get('snr_mae_db') is not None else ""))
    ...
    test_metrics = run_epoch(model, test_loader, loss_fn, optimizer, device, train=False, log_interval=10**9)
    print(f"[INFO] Test: noise_acc={test_metrics['noise_acc']:.4f}, bw_acc={test_metrics['bandwidth_acc']:.4f}, bit_acc={test_metrics['bit_acc']:.4f}" + (f", snr_mae={test_metrics['snr_mae_db']:.2f}dB" if test_metrics.get('snr_mae_db') is not None else ""))

    with open(out_dir / "history.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["epoch", "train_loss", "valid_loss", "train_noise_acc", "valid_noise_acc", "train_bw_acc", "valid_bw_acc", "train_bit_acc", "valid_bit_acc", "train_snr_mae_db", "valid_snr_mae_db"])
        writer.writeheader()
        for h in history:
            writer.writerow({
                "epoch": h["epoch"],
    ...
    summary = {**config, "best_valid_loss": best_valid_loss, "final_epoch": epoch, "total_training_time_s": round(t_total, 2), "test_metrics": test_metrics, "early_stopped": patience_counter >= args.patience}
